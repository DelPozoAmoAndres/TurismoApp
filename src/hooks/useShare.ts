...
export const useShare = (activityId:string) => {
    ...
    const shareOptions = {  //Options to configurate the text and content to share
    ...
    const shareActivity = async () => { //Method to create an intent to share the activity
        try {
            if (Capacitor.isNativePlatform())   //Check if its an native app
