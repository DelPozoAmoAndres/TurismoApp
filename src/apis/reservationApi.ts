...
export const getReservationList = async (): Promise<ReservationGroup[]> => {
  return axios.get(`${process.env.REACT_APP_API_URL}/reservations`).then((res) => {
...
export const createReservation = async (reservation: Reservation, intentId: string) => {
  return axios.post(`${process.env.REACT_APP_API_URL}/reservation`, {
    reservation,
    intentId,
  });
};
...
export const confrimPayment = async (paymentId: string) => {
  return await axios
    .post(`${process.env.REACT_APP_API_URL}/confirm/reservation/`, {
      paymentId,
    })
    .then((res) => res.data);
};
