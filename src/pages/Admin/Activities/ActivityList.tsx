...
const ActivityList: React.FC<RouteComponentProps> = ({ history }) => {
  ...
  const search = (searchText: string, filters: any) => {
    if (searchValidation(searchText))
      getActivityList(searchText, filters).then(activities => setActivities(activities));
  }
  ...
  return (
    <IonPage>
      ...
      <IonContent>
        ...
        <IonItem>
          <IonSelect value={filters.state} placeholder="Filtrar por estado" onIonChange={e => handleFilter(e, "state")} label='Estado' labelPlacement='floating'>
            <IonSelectOption value={null}>Todos los estados</IonSelectOption>
            {states.map((state) => (
              <IonSelectOption key={state} value={state}>{state}</IonSelectOption>
            ))}
          </IonSelect>
          <IonInput type="number" value={filters.duration?String(filters.duration):""} placeholder="Introduce la duración maxima" onIonChange={e => handleFilter(e, "duration")} label='Duración máx' labelPlacement='floating' />
        </IonItem>
        <IonList>
