...
import { createEvent } from '../../../../apis/activityApi';
...
const CreateEvent: React.FC<CreateEventProps> = ({ activityId }) => {
    ...
    const handleAddEvent = () => {
        createEvent(activityId, formData!, filterPropertiesNotNull({ repeatDays, repeatType, repeatStartDate, repeatEndDate, time }))
    };

    const handleRepeatTypeChange = (e: any) => {
    ...
    const handleRepeatDaysChange = (e: any) => {
        ...
        <>
            ...
            <IonItem>
                ...
                <IonSelect value={formData.guide} onIonChange={(e => setFormData({ ...formData, guide: e.detail.value! }))}>
                    {guias?.map((guia) => (
                        <IonSelectOption key={guia._id} value={guia._id}>{guia.name}</IonSelectOption>
                    ))}
                </IonSelect>
            </IonItem>
            <IonItem>
            ...
            {repeatType === 'days' && (
                <>
                    <IonItem>
                        <IonInput value={time} type="time" onIonChange={(e) => setTime(e.detail.value!)} />
                    </IonItem>
                    <IonItem>
                        <IonDatetime locale={defaultLanguage.code} value={repeatDays} presentation='date' multiple onIonChange={(e) => e.detail.value && setRepeatDays(e.detail.value)} />
                    </IonItem>
                </>
            )}
            {repeatType === 'range' && (
                <>
                    ...
                    <IonItem>
                        ...
                        <IonSelect
                            multiple
                            value={repeatDays}
                            onIonChange={handleRepeatDaysChange}
                        >
                    ...
                    <IonItem>
                        ...
                        <IonInput
                            ...
                            onIonChange={(e) => setRepeatStartDate(e.detail.value!)}
                        ></IonInput>
                    </IonItem>
                    <IonItem>
                        ...
                        <IonInput
                            ...
                            value={repeatEndDate}
                            onIonChange={(e) => setRepeatEndDate(e.detail.value!)}
                        ></IonInput>
                    </IonItem>
                </>
            )}
            {repeatType !== "days" && repeatType !== "range" &&
                <IonItem>
                    ...
                    <IonInput
                        ...
                        onIonChange={(e) => { setFormData({ ...formData, date: new Date(e.detail.value!) }) }}
                    ></IonInput>
                </IonItem>}
