    ...
    return (
        <IonItem id="card-search" lines='none'>
            ...
            <IonCard class="ion-no-margin">
                ...
                <IonText><p>{activity.description}</p></IonText>
                <IonCardContent class="ion-no-padding">
                    {auth.user?.role === Role.administrador && !isMobile ?
                        <>
                            <IonButton disabled color={"danger"}  >
                                <IonIcon icon={trashOutline} onClick={async () => await deleteActivity(activity._id!)} />{t("delete")}
                            </IonButton>
                            <IonButton disabled >
                                <IonIcon icon={pencilOutline} />{t("edit")}
                            </IonButton>
                        </>
                        :
                        <IonText>{activity?.events ? <strong>{t("from")}{Math.min(...(activity?.events?.map((e: Event) => e.price as number)!)).toString()}€</strong> : <strong>{t("sold.out")}</strong>}</IonText>
                    }
