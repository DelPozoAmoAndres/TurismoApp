...
export const ReservationActivityData:React.FC<{reservation:Reservation}> = ({reservation}) => {
    const {isMobile} = useScreen(); //Hook to have data of screen dimensions

    return (
        <section className={isMobile ? 'ion-margin-horizontal' : ""}>
            <IonCard >
                <IonImg src="https://imagenes.elpais.com/resizer/2kZjFxiNoG3Pvq9dbeHPTe7aiXc=/1960x1470/cloudfront-eu-central-1.images.arcpublishing.com/prisa/RWF77A5EQGZX4QA2ABH76KQAZE.jpg" style={{ "maxWidth": isMobile ? "none" : 500 }} />
            </IonCard>
            <section hidden={isMobile}>
                <IonCard >
                    <IonCardHeader>
                        <IonCardTitle>{reservation?.activity!.name}</IonCardTitle>
                        <IonCardSubtitle>{formatDate(reservation?.event?.date!, true)}</IonCardSubtitle>
                    </IonCardHeader>
                    <IonCardContent>
                        <IonList>
                            <IonLabel>
