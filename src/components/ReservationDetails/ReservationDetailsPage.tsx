...
interface ReservationDetailsProps extends RouteComponentProps<{ id: string }> { }
                ...
                <section className= {isMobile?'ion-margin-horizontal':""} style={{ "width": isMobile ? "100%" : "auto" }}>
                    <IonCard>
                        ...
                        <div className='ion-margin'>
                            <IonButton routerLink={'/activity/'+reservation?.activity?._id} expand='block'>Ver información de la actividad</IonButton>
                            {reservation?.state==="completed" && <IonButton  expand='block'>Añadir valoración</IonButton>}
                            {reservation?.state==="success" &&  <IonButton color={"danger"} onClick={async () => await cancelReservation(match.params.id)} expand='block'>{t("cancel")}</IonButton>}
                        </div>
                    </IonCard>
                </section>
            </IonRow>
