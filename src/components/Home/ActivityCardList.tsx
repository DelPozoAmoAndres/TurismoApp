    ...
    return (
        <IonGrid class="ion-no-padding">
            ...
            <IonRow>
                <Swiper
                    id='categories'
                    slidesPerView={((width + 10) / (width - 16)) * width / 270}
                    spaceBetween={8}
                >
                    <SwiperSlide>
                        <IonButton mode='ios' color={filtro != "populares" ? "secondary" : "primary"} onClick={() => setFiltro("populares")}>{t("welcome.categories.popular")}</IonButton>
                    </SwiperSlide>
                    <SwiperSlide>
                        <IonButton mode='ios' color={filtro != "montaña" ? "secondary" : "primary"} onClick={() => setFiltro("montaña")}>{t("welcome.categories.mountain")}</IonButton>
                    </SwiperSlide>
                    <SwiperSlide>
                        <IonButton mode='ios' color={filtro != "playa" ? "secondary" : "primary"} onClick={() => setFiltro("playa")}>{t("welcome.categories.beach")}</IonButton>
                    </SwiperSlide>
                </Swiper>
                <Swiper
                    id='cards-home'
