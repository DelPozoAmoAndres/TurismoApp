...
export const ReservationStatusPage: React.FC = () => {
    ...
    return (
        <IonGrid class='ion-padding-horizontal' style={{ "maxWidth": "500px" }}>
            ...
            {canGoBack && <IonButton expand='block' onClick={() => setStep(3)}>{t("try.again")}</IonButton>}
            <IonRow class='ion-justify-content-center'>
                <p>{t("reservation.status.check.reservations")}</p>
            </IonRow>
            <IonButton expand='block' routerLink='/reservas'>{t("go.to.reservations")}</IonButton>
            <IonButton expand='block' routerLink='/home'>{t("go.to.home")}</IonButton>
