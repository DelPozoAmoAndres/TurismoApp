      ...
      <IonGrid id="login-grid" class="ion-no-padding">
        <IonRow>
          <IonList class='ion-margin-bottom'>
            <IonItem lines='none'>
              <IonInput type="email" placeholder={t("personal.data.email")!} value={email} onIonInput={e => { setEmail(e.detail.value!) }} />
            </IonItem>
            <IonItem lines='none'>
              <IonInput type="password" placeholder={t("personal.data.password")!} value={password} onIonInput={e => { setPassword(e.detail.value!) }} />
            </IonItem>
            <IonItem lines='none'>
              <IonText><strong>{t("password.forgotten")}</strong></IonText>
            </IonItem>
            <IonButton type="submit" expand="block" onClick={() => handleLogin()}> {loading ? <Spinner /> : t("log.in")}</IonButton>
          </IonList>
        </IonRow>
        <div>
          <div></div>
        ...
        <IonRow id="social-buttons">
          <IonList mode="ios" class='ion-margin-top'>
            <IonButton size='small' slot='start'>
              <IonImg slot='start' src='https://w7.pngwing.com/pngs/543/934/png-transparent-google-app-logo-google-logo-g-suite-google-text-logo-circle.png' style={{ "width": 20 }} />
        ...
        <IonRow class='ion-padding-top'>
          <IonItem lines='none' routerLink='/registro' onClick={() => modal.current?.dismiss()} style={{ "borderRadius": "5px", "width": "100%" }}>
            <IonText>{t("not.account") + " "}<strong>{t("sign.up.phrase")}</strong></IonText>
          </IonItem>
        </IonRow>
      </IonGrid>
      <IonAlert
        isOpen={showAlert}
        onDidDismiss={() => { setShowAlert(false); if (!error) { modal.current?.dismiss() } }}
        header={error ? t("alert.error.title")! : t("log.in")!}
